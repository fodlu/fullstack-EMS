  ...
  const portalOptions = [
    {
      to: "/login/admin",
      title: "Admin Portal",
      description: "Manage employees, departments, payroll, and company settings.",
      icon: ShieldIcon
    },
    {
      to: "/login/employee",
        ...
        <div className="w-full max-w-md animate-fade-in relative z-10">
          ...
          <div className="space-y-4">
            {portalOptions.map((portal) => (
              <Link key={portal.to} to={portal.to} className='group block bg-slate-50 border border-indigo-400 rounded-lg p-5 sm:p-6 transition-all duration-300 hover:bg-indigo-50 border-indigo-400'>
          ...
          <div className="mt-12 text-center md:text-left text-sm text-slate-400">
            <p>&copy; {new Date().getFullYear()} FodluDev. All rights reserved.</p>
          </div>
        </div>
      </div>
    </div>
