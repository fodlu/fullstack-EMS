...
const Employees = () => {
	...
	const fetchEmployee = useCallback(async () => {
		try {
			const url =
				selectedDept ? `/employees?department=${selectedDept}` : "/employees";
         ...
         w-full sm:w-auto justify-center'>
			...
			<div className='flex flex-col sm:flex-row gap-3 mb-6'>
				...
				<select
					...
					{DEPARTMENTS.map((detpName) => (
						<option key={detpName} value={detpName}>
							{detpName}
						</option>
					))}
				</select>
			...
			{showCreateModel && (
				<div
					...
					<div
						className='relative bg-white rounded-2xl w-full max-w-3xl my-8 animate-fade-in'
						...
						<div className='flex items-center justify-between p-6 pb-0'>
							...
							<button
								onClick={() => setShowCreateModel(false)}
								className='p2 rounded-lg hover:bg-slate-100 transition-colors text-slate-400 hover:text-slate-600'>
								<X className='w-5 h-5' />
							</button>
						</div>
